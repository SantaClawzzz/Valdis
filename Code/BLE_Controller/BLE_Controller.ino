...
void loop() {
  if (deviceConnected) {
    ...
    if (millis() - lastReportTime >= 50) {
      //Analog read joystick
      int16_t rawX = analogRead(JOY_X);
      int16_t rawY = analogRead(JOY_Y);

      //Mapping
      int8_t x = map(rawX, 380 , 3250, -127, 127);
      int8_t y = map(rawY, 300, 3180, -127, 127);

      //Buttons
      uint8_t buttons = 0;
      //BUZZER
      if (digitalRead(BUT) == HIGH) {
        buttons |= 0x01;   // set bit 0
      }

      //ESC
      if (digitalRead(SEL) == LOW) {
        buttons |= 0x02;   // set bit 1
      }
